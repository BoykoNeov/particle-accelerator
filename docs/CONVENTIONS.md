...
- Not claimed **at J2**: sextupole (second-order) detuning (**delivered by O3** —
  `sextupole_detuning`, and `total_detuning` for the sum), the octupole's own
...
## Sextupole amplitude detuning: the second-order normal form (O3 — implemented)

`accsim.twiss.sextupole_detuning(lattice, slices=32)` returns the symmetric 2×2
`[[dQx/dJx, dQx/dJy], [dQy/dJx, dQy/dJy]]` in `m⁻¹`, the same shape and units as J2's
`amplitude_detuning`, and `total_detuning` adds the two. This is the quantity J2's
**Scope** paragraph declared out of scope; that paragraph now points here instead.

An octupole detunes at *first* order in its strength — one phase average of one
potential. A sextupole cannot: its potential is odd in the betatron phase, the
first-order average is exactly zero, and the tune shift only exists at **second** order,
where the ring's sextupoles act in **pairs**. Hence a double sum, quadratic in `k2`.

### The closed form, and what the symbols mean

With `S_i = k2l_i`, `ψ_x = |μ_xi − μ_xj|`, `ψ_y = |μ_yi − μ_yj|`, `Φ = m_x Q_x + m_y Q_y`
and `C(m_x, m_y) = cos(m_x ψ_x + m_y ψ_y − πΦ)/sin(πΦ)`, summing over **all ordered
pairs** `(i, j)` with the diagonal `i = j` counted once, and

    c_i = S_i βx_i^(3/2),        d_i = S_i βx_i^(1/2) βy_i,

    dQx/dJx = −(1/64π) Σ c_i c_j [3 C(1,0) + C(3,0)]
    dQx/dJy = +(1/16π) Σ c_i d_j C(1,0) + (1/32π) Σ d_i d_j [C(1,−2) − C(1,2)]
    dQy/dJy = −(1/64π) Σ d_i d_j [4 C(1,0) + C(1,2) + C(1,−2)]

The four denominators are the lines a sextupole drives: `Q_x` and `3Q_x` from the `x³`
part of its potential, `Q_x ± 2Q_y` from the `xy²` part. That list is not remembered — it
is forced by the monomials: `x³` carries charges `(±3,0)` and `(±1,0)`, `xy²` carries
`(±1,0)` and `(±1,±2)`, and a Poisson bracket lands on the actions only when two charges
cancel, so nothing else can appear.

Every one of those nine coefficients is **verified as an exact symbolic identity** in
`tests/analytic/test_sextupole_detuning.py`, not fitted to a tolerance.

**Two invariances, both gated, both structural rather than cosmetic:**

- Adding an integer to either tune changes nothing — numerator and denominator flip sign
  together. So the full accumulated `μ` and the full tune (integer part included) can be
  used, which is what the implementation does.
- Moving the reference point around the ring changes every `μ_i` and nothing else. That
  is what the `−πΦ` inside each cosine is for: under `ψ → 2πQ − ψ` the cosine is
  unchanged. A formula missing that term passes every single-lattice comparison and fails
  this one.

### Angle and sign conventions, each pinned by a check before use

- **The generator.** `f = −k2l (x³ − 3xy²)/6`, with the bracket
  `[f,g] = f_x g_px − f_px g_x + (y)`. `exp(:f:)z` then reproduces `ThinSextupole.track`
  **exactly** (the Lie series terminates), so the derivation acts on the magnet accsim
  actually has.
- **Conjugating a kick to the reference point** is the `+μ` substitution:
  rotate-by-`μ`, kick, rotate-back `= exp(:f∘R(+μ):)` exactly. The `−μ` variant is off by
  0.93 on the same test case.
- **Composition.** Applying `exp(:a:)` then `exp(:b:)` equals `exp(:a + b + ½[a,b]:)`.
  The `+½` is *solved for*, then re-checked against an explicitly composed two-sextupole
  turn: residual `2.2e-16` at `+½`, `0.91` at `−½`, `0.46` at `0`.
- **Reading the tune out** of the normal form: `2π ΔQ_u = −∂N/∂J_u`. Fixed by requiring
  the machinery to reproduce the quadrupole's `βk1l/(4π)` and the octupole's
  `k3l βx²/(16π)`.

### The anchor that matters: two quadrupoles against the exact trace

The load-bearing check is *not* the octupole (that is first order and only pins the
read-out). It is **two thin quadrupoles**, where the second-order tune shift is available
from linear algebra alone — expand `cos 2πQ = ½Tr(M)` of the real one-turn matrix — and
must equal what the perturbation machinery produces. Written in the unit-modulus symbols
`A = e^{2πiQ}` and `a = e^{iμ}` both sides are rational functions, so the agreement is
`cancel(...) == 0`: an **identity**, not a tolerance.

Run in **both beam orderings**, because a derivation that produced `cos(Δμ − πQ)` instead
of `cos(|Δμ| − πQ)` agrees with one ordering and fails the other. This one anchor pins
the resonance denominator, the `πQ` inside the cosine (as against `2πQ` — the commonest
transcription error in this family, and one that survives a numeric spot check at a
generic tune), and the sign of second order relative to first.

**What it cannot reach:** a quadrupole drives no `3Q_x` line, so `C(3,0)` is not anchored
there. It is gated instead against PTC next to the third-integer resonance.

### PTC's action convention: `anhx` is `dQ/d(2J)`, half of ours

Measured, not recalled — calibrated against the octupole formula both codes already
agree on. A comparison written without the factor is wrong by exactly two and reads like
a missing `½` in the derivation.

### The exact drift detunes with no magnets at all, in both codes

PTC with `exact=true` gives its drift `x += L px/pz`, which is nonlinear, so the ring
detunes with every multipole switched off: `0.127` on the O3 fixture, against `0.542` for
the sextupoles — a quarter of the signal, not a rounding contaminant. accsim's `Drift` is
exact too (see the M2/M3 drift-model note) and does the same under tracking.

`sextupole_detuning` reports **exactly zero** there, correctly: it is the sextupoles'
contribution, not the ring's total anharmonicity. **Every comparison against a tracked or
PTC number must therefore be a difference against the same ring with `k2 = 0`** — the
shape J2's tracked gate already used for a different reason.

### The agreement with PTC is exact, and a pre-committed gate had the wrong shape

O3's roadmap pre-commitment named the primary gate as an exponent scan: PTC is
all-orders, the closed form is second order, so at any fixed strength they must disagree
and the relative residual should fall as `k2²`. **It does not, and the reason is worth
recording rather than deleting.** PTC's `anhx(1,0,0)` *is* the coefficient of the term
linear in the action, i.e. the quartic part of the normal form — the same object this
package computes. Raising `no` adds higher powers of the action (`anhx(2,0,0)` and
friends); it does not correct this one, and `no = 4, 5, 6` return bit-identical values.

So once the kinematic baseline is subtracted the two agree to round-off: ratio
`1.0000000000` on all three independent entries, on three working points. The exact
comparison is strictly stronger than the scan, and the scan survives where all orders
genuinely do enter — the **tracked** gate.

The flip side, stated because it changes what the evidence means: both sides now compute
the same object by the same kind of procedure, so PTC alone is not independent
confirmation of the physics. The independent legs are the two-quadrupole exact-trace
anchor and the tracked measurement.

### The tracked residual falls as an ODD power of amplitude

J2's octupole gate watches its residual fall by **16** per amplitude halving (quadratic in
the action). This one falls by **8**, which looks impossible for a tune — a tune is even in
the amplitude. The cause is not the formula: the prediction is evaluated at the
**Courant-Snyder action of the launch point**, and once a sextupole is present that is not
the particle's invariant action; it is wrong by a phase-dependent `O(k2 x³)`.

Both halves are measured. At fixed Courant-Snyder action the tracked detuning varies by
**±2.1 %** across launch phase at 2 mm and **±1.1 %** at 1 mm — the spread is *linear* in
amplitude, as that explanation requires and as nothing else would be. Averaging over the
launch phase restores the expected order exactly: residual ratios **16.02, 16.00, 16.00**.

Practical consequence for anyone measuring `dQ/dJ` by tracking here: launch at several
phases and average, or accept a first-order-in-amplitude error.

### A thick sextupole approaches a thin one LINEARLY, not quadratically

J2 gates the thick octupole's limit as `L²`. The sextupole's is `L` — measured 2.08, 2.04,
2.02 for successive halvings — and a test written by analogy with J2 fails for a reason
that is physics. The pair kernel carries `cos(|Δμ| − πQ)`, whose `|·|` has a **kink** at
zero phase separation, so the mean `|Δμ|` between two slices of one body is first order in
the body length. The octupole's contribution is a smooth single sum over `β²` and has no
such kink.

The slicing itself is the midpoint rule and converges at **second** order in the slice
length (ratios 4.05, 4.01, 4.00, 4.00) — a different limit, and the one that says the
slice placement is right.

### The `3Q_x` term is not a small correction on a short ring

The received picture is that `C(3,0)` matters only near `Q_x = n/3`. On the O3 fixture (a
12 m ring, four cells) it is already **3.6×** the `C(1,0)` part at a generic working point,
because the phase advances between sextupoles are a large fraction of the whole turn.
Recorded so that a future "it's negligible away from the resonance" is checked rather than
assumed.

### Fixture degeneracies that would hide a wrong term

- **`Q_x = Q_y`** makes `Q_x + 2Q_y = 3Q_x` and `Q_x − 2Q_y = −Q_x`: the two coupled lines
  collapse onto the two uncoupled ones and a wrong cross-plane term becomes invisible. The
  ring the PTC arbiter was first checked on had exactly this (`Q_x = Q_y = 0.126531`,
  forced by `|kf| = |kd|` in a symmetric FODO). Every O3 fixture has `Q_x ≠ Q_y`.
- **Weak `β` contrast.** The `β^(3/2)`-per-generator weighting is reached by moving one
  sextupole and *predicting* the change. The first version of that gate compared positions
  whose `β_x` differed by **4 %**, where `β²` and `β³` are indistinguishable — nearly
  vacuous. It now runs from the `β_x` minimum to the maximum (41 % contrast, a factor 2.8
  in the prediction) and explicitly *excludes* the neighbouring exponents.

### Sextupoles and octupoles together: no measurable cross term

`total_detuning` adds J2's first-order octupole matrix to O3's second-order sextupole one
**unadjusted**, and PTC — which reports the true anharmonicity of the ring carrying both —
agrees to nine digits. So at this order the two mechanisms do not interfere.

That statement is only worth anything at strengths where both terms matter. The gate is
run where they are comparable and where the cross entry very nearly **cancels**
(`-2.617 + 2.346`), leaving a sum a tenth the size of either piece, so an error in either
formula is amplified. Run instead at a strength where the octupole term is `10^4` times
the sextupole one (the first version of this gate), the sum agrees with PTC whatever the
sextupole formula says.

### The resonance guard

On one of its own driven lines the second-order normal form does not exist: the
perturbation series has a zero denominator, and the tune of a finite-amplitude particle is
not an analytic function of its action at all. `sextupole_detuning` raises
`ResonantLatticeError` (a new sibling of `UnstableLatticeError` and `CoupledLatticeError`)
rather than dividing, at `|sin πΦ| < 1e-12`. The divergence *approaching* a line is
...
Do not push a near-resonance comparison closer than the codes agree on the **tune**:
`1/sin(3πQ_x)` amplifies a tune disagreement by `1/|Q_x − 1/3|`, so the `1e-9` accsim and
MAD-X share becomes `2.5e-7` at `4e-3` and would be `1e-3` at `1e-6`.

### Out of scope for O3

Second order in `k2` **exactly**, on the **design** orbit, for a transversely uncoupled
lattice (`CoupledLatticeError` otherwise, via `closed_twiss`). Skew sextupoles are
ignored. Not included: the octupole's own second-order term, `k2⁴`, detuning quadratic in
the action (`d²Q/dJ²`), the kinematic detuning of the exact drift, resonance driving terms
with phase (that gap is closed — see *Resonance driving terms* below, which also decodes
PTC's `gnfu` indexing), and dynamic aperture. Thick sextupoles are sub-sliced, and the double sum is
materialised, so cost and memory are `O((N·slices)²)`.

## Resonance driving terms: the same normal form, one order earlier (O4 — implemented)

`accsim.twiss.resonance_driving_terms(lattice, slices=32)` returns a
`dict[str, complex]` with the seven first-order terms a **normal sextupole** and a
**skew quadrupole** drive, keyed the way the field keys them:

    f3000  f2100  f1020  f1011  f1002      (sextupole, k2l)
    f1010  f1001                           (skew quadrupole, k1sl)

Those seven are *all* of them: `F` is a real function, so `f_jklm = conj(f_kjml)`, and
the sextupole's ten cubic monomials and the skew quadrupole's four are these seven plus
their conjugates.

### What the number is: what O3 threw away

O3 built the normal form of the one-turn map and kept the part depending only on the
actions — the tune shift. Removing each *non*-action monomial costs one Lie generator,
and the coefficient of that generator **is** the driving term. O4 is therefore not a new
calculation: it is the intermediate quantity `_homological` already produces inside
`tests/analytic/test_sextupole_detuning.py`, read out instead of discarded. The O4 test
file imports that machinery rather than restating it, which is what makes O3's four
pinned conventions (the generator, the `+μ` conjugation, the resonance basis, the
bracket) carry over without being re-argued.

Writing the one-turn map as `exp(:F:)` then the linear rotation, and expanding `F` in the
resonance basis,

    F = Σ_jklm F_jklm h_x^j conj(h_x)^k h_y^l conj(h_y)^m ,

    f_jklm = F_jklm / (exp(−2πi[(j−k)Q_x + (l−m)Q_y]) − 1) .

Each source contributes, with `E(m_x,m_y) = exp(−i(m_x μ_x + m_y μ_y))` at the source and
`β`, `μ` the **unperturbed** optics there:

    sextupole, S = k2l                     skew quadrupole, K = k1sl
    F_3000 = −S βx^(3/2) E(3, 0) / 48      F_1010 = +K √(βx βy) E(1, 1) / 4
    F_2100 = −S βx^(3/2) E(1, 0) / 16      F_1001 = +K √(βx βy) E(1,−1) / 4
    F_1020 = +S √βx βy   E(1, 2) / 16
    F_1011 = +S √βx βy   E(1, 0) / 8
    F_1002 = +S √βx βy   E(1,−2) / 16

Every one of those nine numbers is verified as an **exact symbolic identity** against
`_homological`'s own output — `cancel(...) == 0`, not a tolerance.

The charge `(j−k, l−m)` is simultaneously the phase the term carries and the line it is
divided by, so the list of driven resonances is *forced* rather than remembered: `3Q_x`
and `Q_x` from a sextupole's `x³`, `Q_x` and `Q_x ± 2Q_y` from its `xy²`, `Q_x ± Q_y` from
a skew quadrupole's `xy`.

### The basis is `h = û + i p̂`, and tracking — not a code — decides it

`h_u = û + i p̂_u` (so `h_u = √(2J_u) e^{−iφ_u}`). The opposite basis, `û − i p̂_u` — the
one O3's derivation is carried out in — gives the **complex conjugate** of every term.
The two agree in modulus, so *any* magnitude-only comparison is blind to the choice: a
package could ship the mirror image of what everyone else calls `f3000` and pass every
`abs()` gate ever written. This is O1's "the content is the PHASE" in a new costume.

accsim ships the `+i` basis, which is xtrack's and MAD-X's. That is measured, not
declared, in three places: xtrack agrees to `1e-10`, PTC agrees to `1e-14`, and — the
one that is not a convention argument at all — the **tracked** sideband phases agree to
`1e-4`, with the conjugate excluded by a factor of a thousand.

### An RDT is covariant, not invariant — and that is the sharp gate

O3's detuning is a property of the ring: move the observation point and it does not
change at all. An RDT is a property of the ring **and the point it is read from**. Moving
the start forward through phase advances `(d_x, d_y)`, past a set of sources,

    f_new = exp(+i (m_x d_x + m_y d_y)) · (f_old + F_crossed) ,

with `F_crossed` the sum of the **plain, undivided** `F` coefficients of just the sources
stepped over. So each term rotates between sources and **jumps** at them; `|f|` is not
constant around the ring (it varies by more than 1.5× on the O4 fixture). A wrong
conjugation, a wrong denominator or a missing phase all reproduce a single-point
comparison and all fail this one, so the roll test is written with shifts crossing zero,
one, two and five sources — which separates the rotation from the jump instead of
measuring them together.

Both halves are also checked against a second code. xtrack reports RDTs along the ring;
rolling accsim's element list to each of its observation points reproduces its column at
`1e-8` — which incidentally establishes that both codes' rows sit at the element
**entrance**, since a quadrupole and a drift have length and agreeing at one end excludes
the other.

### At a thin source the two codes report opposite sides of the jump

Found, not expected, and it looks like a phase error if you have not seen it. An RDT is
discontinuous at a thin source, so "the value at element *i*" needs a side, and xtrack's
row for that element is **downstream** of the kick while rolling accsim's list so the
source comes first observes **upstream** of it. Everywhere else in the ring the two agree
to round-off, so a comparison written without this fails at exactly the source elements —
three points out of twenty-two on the fixture.

What makes it a gate rather than a caveat is that the step is *predicted*: crossing one
source at zero phase advance adds precisely its plain `F` — coefficient, strength, beta
powers, no resonance denominator. Checked on all five sextupole terms against xtrack, to
`3e-11` absolute on steps of order one.

### G1's `|C⁻|` is the one leg that shares no algebra

`closest_tune_approach` has been in the package since G1 and was derived from the exact
eigen-tune split of a single skew kick — a different route with nothing in common with
the Lie machinery. It knows only the modulus, which makes it a magnitude leg; but the
magnitude is exactly where a factor of two or of `2π` would hide, which is what the basis
choice risks.

    |f1001| · 4 |sin(π(Q_x − Q_y))| = π |C⁻|

The prefactor was **measured before it was asserted**: computed on three rings with
different tunes, different skew positions and different strengths (including one four
orders weaker), checked to agree with each other first, and only then pinned. It comes
out `π` to round-off at every strength, because both sides evaluate on the same
unperturbed optics and neither is an approximation of the other.

### The tracked sidebands, and how the launch is divided out

An RDT is a named line of the turn-by-turn spectrum. Derived from the same machinery,
not quoted: first-order perturbation gives `h = ζ − 2i ∂G/∂conj(ζ)`, so with
`A(ν)` the complex amplitude of the `exp(−2πiνn)` component,

    A_x(Q_x)   = √(2J_x) e^{−iφ₀}
    A_x(−2Q_x) = 12 i J_x conj(f3000) e^{+2iφ₀}   ⇒  f3000 = conj[ A_x(−2Q_x) / (6i conj(A_x(Q_x))²) ]
    A_y(Q_x)   = 2 i f1001 √(2J_x) e^{−iφ₀}        ⇒  f1001 = A_y(Q_x) / (2i A_x(Q_x))
    A_y(−Q_x)  = 2 i conj(f1010) conj(A_x(Q_x))    ⇒  f1010 = conj[ A_y(−Q_x) / (2i conj(A_x(Q_x))) ]

All three read-outs are **ratios**, so the action and the launch phase cancel and there
is nothing about the launch that could be tuned to make the comparison work. The only
monomial contributing at each frequency is the one named, which is why the lines are
clean. `f1001` and `f1010` come off the **same trajectory**, on the two sidebands at
`±Q_x`, which is also what tests the sign of `m_y`: the sum and difference resonances
differ only in that sign, and swapping them moves each answer onto the other's line.
They are separated by the fact that the ordering `|f1010|/|f1001|` **flips** between two
working points (`0.17` at one, `1.8` at the other) — a check that survives no shared
error, and one written from the measurement after the first guess about which is larger
turned out backwards.

Two residuals, separated rather than lumped: at long turn counts the `f3000` error is
**quadratic in the launch amplitude** (the second-order RDT this milestone does not
compute), and at fixed amplitude it falls with turn count until it reaches that floor
(spectral leakage from the primary line, which more turns remove). Measured `7.5e-6`,
`1.9e-6`, `5.1e-7` at `4e-4`, `2e-4`, `1e-4` m.

A control matters here: the exact drift is nonlinear and detunes the ring all by itself
(the O3 finding), but its nonlinearity is *even* in the momenta, so it reaches `2Q_x` and
`4Q_x` and **not** the third integer. On a sextupole-free ring the `−2Q_x` line is below
`1e-6` of the primary — which is what stops the tracked gate from reading the lattice
instead of the sextupoles.

### The residual against a coupled reference is CUBIC in the skew strength

Measured, and one power better than the obvious guess. `f1001` is *linear* in `k1sl`, so
a naive reading expects the first correction at the square. It is not: the **relative**
gap to a coupled reference falls by four for every halving of the strength, i.e. the
absolute gap is cubic. The cause is what actually perturbs the answer — accsim evaluates
first-order theory on the *unperturbed* optics, and the optics' own shift from coupling
is itself quadratic. The same exponent, from the same cause, appears in both the tracked
gate and the xtrack comparison (`5.3e-2` relative at full strength on the O4 fixture,
`4.00` per halving).

### First order in `k2` is EXACT here, unlike O3's detuning

The natural expectation after O3 — an all-orders code against a first-order formula must
disagree somewhere, and the gap should grow with strength — is **wrong**, and the reason
is structural. The bracket of two cubic generators is *quartic*, so a sextupole's
second-order contribution lands on the amplitude detuning O3 computes and never returns
to these cubic coefficients. PTC agrees at `1e-14` with the sextupoles at `0.3×`, `1×` and
`3×` strength, and `no = 4` and `no = 6` return bit-identical values.

This is the mirror image of O3, and worth keeping the pair in mind: a sextupole's
*detuning* does not exist at first order at all, while its *cubic driving terms* are
complete at first order.

### PTC's `gnfu` indexing, decoded — and why it looked impossible

The O3 session found `gnfa`/`gnfc`/`gnfs` and could not say which RDT each was; the O4
candidate recorded the indexing as undecoded and "a session of its own". The difficulty
was two things at once, neither of them physics:

- `select_ptc_normal, gnfu=...` takes **four** indices and `normal_results` carries
  `order1` … **`order4`**. Reading only the first three drops the last index, which makes
  the returned keys look unrelated to the request.
- PTC returns a handful of **empty rows whose keys were never requested and are not even
  cubic** (`(0,0,2,0)`, `(1,0,0,0)`, `(2,0,0,0)` on this fixture), interleaved with the
  real ones. Truncated keys plus junk-keyed blanks is exactly the reported "five entries
  keyed `(1,0,0)`, `(1,0,1)`, `(1,0,2)`, `(2,1,0)`, `(3,0,0)` for three requests".

With the fourth column read and blanks recognised by their **value** rather than their
key, the mapping is trivial — PTC's key is `(j, k, l, m)`, accsim's own indices — and the
only remaining difference is a factorial:

    gnfc(j,k,l,m) + i · gnfs(j,k,l,m)  =  j! k! l! m! · f_jklm ,   gnfa = its modulus.

That is measured from the *pattern*, not from one number: the five terms carry the four
distinct weights `6, 2, 2, 1`, which a single wrong overall constant cannot reproduce.

Two traps for the next person: PTC returns **far fewer rows than were requested**, so
absence from the table is not evidence a term is zero; and a row must be recognised by
its value rather than by trusting its key.

### Which optics: unperturbed, and why the guard is G1's

First-order perturbation theory is evaluated on the **coupling-off** optics, the way
`closest_tune_approach` does — a skew quadrupole makes `closed_twiss` refuse outright,
and the theory wants the unperturbed optics anyway. That is also what makes the `|C⁻|`
tie exact rather than asymptotic.

The sum walks element *types*, so it inherits G1's hazard and G1's guard: an element that
couples the planes **without being a skew quadrupole** — a rolled quadrupole, say — would
make this report `f1001 = f1010 = 0` for a ring that is demonstrably coupled. It is
...
stretches are available. The `_fodo` helper now **asserts that every requested source was
actually placed**, and that assertion failed on its first run: two of the three skew
quadrupoles in one comparison ring were silently landing inside quadrupoles and vanishing,
so a gate written as a three-source comparison was running with one. This is the same
...
the offending line named in the message. *Approaching* a line the term genuinely diverges
and that divergence is returned: near `Q_x = 1/3`, `f3000` grows as `1/|sin 3πQ_x|` (a
factor-four step in the distance gives a factor-four step in the divergence, gated) while
`f2100` and `f1011` — which carry `Q_x`, not `3Q_x` — barely move. That per-term
fingerprint is what checks each coefficient was divided by *its own* line, which no
single-ring comparison can see.

### Out of scope for O4

First order in the strengths; normal sextupoles and skew quadrupoles only; on the
**design** orbit (no feed-down from a closed orbit or a misalignment); at the **lattice
entrance** (roll the element list to observe elsewhere). Not computed: octupole terms
...
## The solenoid: the magnet whose field points along the beam (S1 — implemented)

`Solenoid(length, ks, ...)` in `accsim.elements.solenoid`, exported at the package top level.
The first element in the package whose field is **longitudinal**, and the only one that
couples `x` and `y` without being a rolled version of something else.

**Strength.** `ks = B_s / (B rho)_0` [rad/m] — MAD-X's `KS`, xtrack's `ks`.

**`ks` is charge-free, and it was measured rather than assumed.** An electron (`q0 = -1`) and
a proton (`q0 = +1`) at the same `gamma0` and the same `ks` get **bit-identical** maps in
xtrack (`max|diff| = 0.0`) and identical `re<ij>` in MAD-X. A solenoid's coupling sense is
physically set by the sign of the charge, so the sign could plausibly have lived in the
reference particle instead; both codes put it in `ks` itself. **`_matrix_body` therefore never
reads `ref.charge`**, and both `tests/analytic/test_solenoid.py` and
`tests/reference/test_solenoid_madx.py` gate it. Getting this wrong would flip the coupling
for every electron machine while leaving every proton machine right — the shape of error a
single-species suite never sees.

### The map

From the paraxial Hamiltonian with the uniform solenoid's normalised vector potential
`a = (-ks y/2, +ks x/2)` (whose curl is `ks` along `s`), sympy's `dsolve` returns

    M4 = Rot(K L) . blockdiag(F(K^2, L), F(K^2, L)),      K = ks / (2 (1 + delta)),

with `Rot(theta)` turning `(x, y)` and `(px, py)` together and `F` being
`quadrupole._focusing_block` — the **quadrupole's own** 2x2 — at strength `K^2`, *equal in
both planes*. Written out, with `C = cos KL`, `S = sin KL`, on `(x, px, y, py)`:
...
**The `1/2` is the element.** A charge in a longitudinal field precesses at the *cyclotron*
rate `ks`; its **orbit** rotates at the *Larmor* rate `ks/2`. The factor is derived from the
Hamiltonian in the analytic suite (the solve is checked by `M4(L = pi/ks) = -I`, a half orbit
turn), not recalled — a cyclotron-rate solenoid misses MAD-X by `O(1)` and that control is a
test of its own.

`R56 = L / gamma0^2`, a straight element's, as for a drift and a quadrupole.

**Agreement.** MAD-X entrywise at **`2.2e-16`**; xtrack's `UniformSolenoid` tracked, at the
paraxial floor below. `xt.Solenoid` is deprecated in favour of `xt.UniformSolenoid`; both were
probed and give the same map.

### What it actually does: it rotates the velocity, it does not focus it

Conjugating the matrix by the shear `J` that turns a canonical momentum into a geometric angle
*inside* the magnet, `x' = px/(1+delta) + K y`, gives

    [[1, sin(2KL)/2K, 0, sin^2(KL)/K],
     [0,    cos 2KL,  0,   sin 2KL  ],
     [0, -sin^2(KL)/K, 1, sin(2KL)/2K],
     [0,   -sin 2KL,  0,   cos 2KL  ]]

— **the angle rows contain no position at all.** The transverse velocity is rigidly rotated
through `2KL` with its magnitude untouched, and every focusing entry in the canonical matrix
is the projection of that turning velocity onto position. Two consequences, both gated:

- **`x'^2 + y'^2` is exactly constant** through the body (`d/ds = 0`, derived), so the path
  lengthening is a **drift's**, `L (x'^2 + y'^2) / 2` — evaluated at the angles the particle
  has *inside* the magnet.
- **That shear is the hard edge.** The fringe field at each face is what keeps the canonical
  momentum continuous while the geometric angle jumps, so the boundary-to-boundary matrix
  above already has *both* faces in it — which is exactly why it is a drift at `ks = 0`. Code
  that treated `px` as the angle would have silently dropped both faces; the resulting map is
  still symplectic and still a drift at `ks = 0`, so neither of those gates can see it, and it
  is wrong at **first order in `ks`** (asserted by halving `ks` and watching the gap halve).

### Exact in `delta`, paraxial in the angles

`matrix()` is evaluated at `delta = 0`; `track()` uses the per-particle
`K = ks/(2(1+delta))`, because `ks` is normalised to the *reference* rigidity. The chromatic
law is gated as an **identity**, not a tolerance: the tracked transverse map equals
`D M4(ks/(1+delta)) D^-1` with `D = diag(1, 1+delta, 1, 1+delta)`, to `1e-15`.

What is left over against xtrack is P2 (iv)'s **kinematic** remainder — the paraxial expansion
of `sqrt((1+delta)^2 - px^2 - py^2)` — and it is identified by measurement, not assumption:
halving the transverse amplitude divides the residual by exactly **8.000** (four amplitudes),
i.e. it is cubic. Sizes on the probe state: floor **`1.1e-10`**; a frozen-`K` (momentum-blind)
map misses xtrack by `6.2e-8` / `6.2e-7` / `6.2e-6` at `delta = 1e-4` / `1e-3` / `1e-2`, i.e.
**5.6e2 / 5.6e3 / 5.7e4 times** the floor. The momentum dependence is the effect, not a
refinement below the noise.

**`kinematic_slices` is deliberately not offered** on `Solenoid`, unlike on `Quadrupole`. In a
solenoid the kinematic remainder is a function of the **mechanical** momenta, not the
canonical ones, so it is *not* the momentum-only drift `quadrupole.kinematic_drift`
integrates, and the symmetric split P2 (iv) uses does not apply unchanged. The gap is
recorded, gated by its cubic scaling, and left open.

### Axial symmetry

A solenoid is invariant about its own axis, so **`roll` is a no-op — bit-for-bit**, at any
angle. Nothing else in the package can be tested this way (a rolled quadrupole is a skew
quadrupole, a rolled sextupole a skew sextupole, a rolled bend is the whole of K2), and the
assertion gates the alignment conjugation at the same time. A transverse **displacement** is
ordinary K1: `matrix` untouched, the whole effect the constant `(I - M) d` — which for a
solenoid puts a *vertical* kick on a purely horizontal offset, the coupling showing up where
it should.
...
- **`closest_tune_approach` and `resonance_driving_terms` raise `CoupledLatticeError`**, and
  ...
  `f1001 = 0` for a ring that is demonstrably coupled. Their **measured** guard (an element
  whose own matrix has a nonzero transverse off-block and is not a skew quad) catches it — the
  solenoid is the first element other than a rolled magnet to trip it.
- **The uncoupled Courant-Snyder path (`tunes`, `twiss`) raises**; the **eigen path works with
  no change at all**: `normal_mode_tunes` and the Edwards-Teng `coupled_twiss` diagonalise the
  one-turn map and see a solenoid exactly. That is the whole of what G1/G2 built, arriving for
  free, and the ring-level agreement with xtrack's coupled Twiss is **`9.1e-14`** on the
  normal-mode tunes — round-off, and far below the single-element tracking floor above,
  because a Twiss is built on the *linear* map where the two codes' solenoids agree exactly.

### How much a solenoid couples a ring, and the fixture that can say so

**`1 - gamma_c` grows as `ks^2`** in the weak regime — `C` is first order in `ks`, and
`gamma_c^2 + det C = 1` makes the mixing angle's cosine second order in it. Measured ratios
for successive doublings from `ks = 0.0125`: **3.9957, 3.9828, 3.9326**, converging on 4 from
below.

**That law is only visible off the difference resonance, and the obvious fixture is on it.**
A bare 4-cell FODO has `Q_x = Q_y` *exactly*; there, *any* coupling mixes the modes fully
(`gamma_c -> 1/sqrt(2)` for `ks = 0.05` as much as for `ks = 1`) and the mixing then
**decreases** with `ks`, because the solenoid's own focusing pushes the tunes apart. Both the
analytic and the xtrack ring fixtures therefore carry a weak quadrupole that splits the tunes,
and it is not cosmetic: without it a correct element fails a "mixing grows with `ks`" gate,
and mode-to-mode matching between two codes is meaningless because the modes are degenerate.

A solenoid has **no closed-form `|C^-|`** in this package — the perturbative sum refuses it,
above — so the `ks^2` law plus the exact `ks = 0` reduction is the whole of what the analytic
suite can assert, and the absolute value is xtrack's business.
- **`survey` is blind to `ks`** — a solenoid is straight, and R1's walk reads `length` and, for
  a bending element, `angle`. Bit-identical tables.

### Tapering

**A solenoid is a powered magnet and a taper scales `ks`.** `tapering._scaled` raises for an
element on neither of its lists, so shipping this element *forced* the decision rather than
allowing a silent default — which is why that raise exists. `ks` is normalised to the
reference rigidity exactly as `k1` is, so a beam that has sagged to `1 + delta` is
**over-rotated** by a design-strength solenoid in the same way it is over-focused by a
design-strength quadrupole. Gate: Q2's fixed point — a solenoid whose `ks` has been scaled by
`1 + delta` has, at that momentum, exactly the design magnet's map.

**But `taper()` cannot yet be applied to a ring containing a solenoid, and that is the price
of the refusal above.** A taper profile needs the ring's *radiating* closed orbit; radiation
reaches an element through `normalized_field`; that refuses. So the classification and the
fixed point are gated directly (on `tapering._scaled`), and the end-to-end `taper()` call is
gated as a **refusal**, with a test that fails loudly the moment S2 gives the accessor an `s`
component — at which point it becomes the end-to-end gate with no other change. The trade is
still the right one: a silent zero costs a *wrong* answer on the spin side, where this costs
only a missing one.

### The second-order map, unasked

P1's `taylor_expand` differences `track` and knows nothing about element types, so a solenoid
arrives with a `T` tensor for free — and its content is the milestone restated as structure:
**every** non-negligible entry either carries a `delta` index (the chromatic term) or lies in
the `zeta` row (the path lengthening), and nothing else is second order, because the
transverse map is exactly linear at fixed momentum. Largest entry outside those two sets:
`5.4e-14`, the differencing floor. Largest inside: `L/2`. The second-order symplectic identity
holds in **both** longitudinal pairs — unlike the sector bend's, which fails it in
`(zeta, delta)` by a closed form — because a solenoid's `R51..R54` are zero: its path
lengthening is quadratic, not linear.
...
- **Tracking with any radiation model** other than `"off"` raises, through the field accessor.
  So does a spin track, for the same reason. **Radiation lifted by T3**, which gave the field
  an `s` (`Element.field_at`); the spin half is still refused and is T4.
- **`taper()` refuses**, and *which* refusal fires first was measured rather than predicted:
  ...
  now has exactly one cause, the accessor. **T3 removed that one too.**
...
### What T2 still refused, and what T3 did with it

**`radiation_kick` through a wiggler raised.** The accessor had no `s` and the field reverses
`2 * periods` times inside the element, so a single mid-point sample could not see it. The
consequence, stated rather than implied: a wiggler **radiated in the integrals and not in
tracking**, so `taper()`, radiation tracking and spin tracking all refused. T2 priced closing
that as "an `s`-dependent accessor plus a slicing model, a milestone of its own" — and **T3
did it, with a different architecture from the one that phrase implies**: the sampling is
resolved, the map is not. Radiation tracking and `taper()` work; spin is still refused, and is
T4. See *The wiggler radiating in tracking* below.
...
## The wiggler radiating in tracking (T3 — implemented)

T2 left the package radiating a wiggler on the **design** route and not on the **tracked** one:
`radiation_integrals` carried `<h^2>` and `<|h|^3>`, while a particle crossing the same magnet
lost nothing, because `radiation_kick` samples the field once per traversal and
`Wiggler.normalized_field` raised rather than answering the zero that sample would give. T3
closes it, and the closing is smaller than its own description suggested.

### Slice the sampling, not the map — and T1 is the reason

T2's own summary priced the fix as "an `s`-dependent accessor plus a **slicing model**", which
reads as *cut the element into sub-elements and track through each*. That construction is
wrong here. The wiggler's vertical focusing exists **only** as a period average:
`y'' = -h0^2 y sin^2(ks)` is Mathieu-like and has no closed form on a partial period, and a
sub-period piece of the shipped map has no wiggle in it at all (horizontally the map is a
drift, whose `x'` is constant). So T3 resolves the **radiation sampling** and leaves the
transfer map exactly as T1 shipped it. `matrix()`, `kick()` and every optics quantity are
bit-identical, and `radiation_slices` cannot move one of them.

### The seam: `Element.field_at(s, x, y)` and `radiation_sample_positions()`

Two new hooks on `Element`, both defaulting to today's behaviour:

- **`field_at(s, x, y) -> (bx, by, bs, ax, ay)`** — the field *and* the vector potential at
  `s` metres into the body, in the element's own frame. The default composes the three
  `s`-independent accessors at the same arguments and ignores `s`, so every element built
  before T3 answers with the same arithmetic. `s` broadcasts against `x`/`y`: a column of
  sample positions against a row of particles returns the grid.
- **`radiation_sample_positions() -> ndarray | None`** — `None` (the default) is the single
  mid-point sample. An override returns `s` positions, each standing for an **equal** share
  of the traversal's path length; the loss, the excitation variance and the photon count are
  each **summed** over them, never averaged.

This is the fourth interface finding on this line: S1 found no place for a field *along* the
beam, S2 none for the *vector potential*, T1 none for a field that varies along the magnet,
T3 gives that one a home.

### The wiggle angle *is* the vector potential

A planar wiggler's whole field comes from one component,

    a_x = (h0/k) sin(ks) cosh(ky) = theta sin(ks) cosh(ky),   a_y = 0,

since `curl a` gives `b_y = da_x/ds` and `b_s = -da_x/dy`. The consequence is the
implementation: the state vector carries **canonical** momentum, the physics wants the
**kinetic** one, and `px - a_x = px - theta sin(ks)` on axis is *exactly* the sub-period orbit
`x'(s) = -theta sin(ks)` that the period-averaged map deliberately does not carry. That is
**why** the shipped map holds `px` constant through the magnet — the wiggle lives in `a`, not
in `p` — and it means `radiation_kick` reconstructs the sub-period orbit through the `p - a`
subtraction S2 already put there, with no orbit-reconstruction code at all.

The transverse **position** excursion `x(s) = x0 + (theta/k)(cos ks - 1)` is equally absent
from the state and does **not** need synthesising: `b_y` carries no `x` dependence, so it
moves the curvature by nothing. Only the angle enters, and only through the perpendicular
projection.

**And that projection barely notices it.** The wiggle is horizontal and `b_y` is vertical, so
the velocity is *always* perpendicular to the field in the plane of the motion: `|B_perp|`
does not contain the wiggle angle. Deleting `a` changes the on-axis loss by **not one bit** at
any `delta`, and moves it by `5.794e-07` at `y = 2 mm` (through `b_s`, which vanishes on the
mid-plane). The finding is elegant and, for radiation, nearly inert; where the wiggle angle
will matter is the spin precession axis, which is T4. Both ends are gated — the bit-exact zero
and the `5.8e-07`.

### Three moments, three convergence rates, and the headline gate is the vacuous one

Sampling the field at the mid-points of `n` uniform steps **per period**:

| quantity | integral | order in `h0` | convergence |
|---|---|---|---|
| mean energy loss | `int kappa^2 ds` | `h0^2` | **exact at every `n >= 3`** |
| photon count | `int abs(kappa) ds` | `h0` | `n^-2` (measured 4.01, 4.00, 4.00) |
| excitation variance (`I3`) | `int kappa^3 ds` | `h0^3` | `n^-4` (measured 16.51, 16.12, 16.03) |

The mean is exact because `cos^2 = (1 + cos 2ks)/2` and the uniform mid-point sum of
`cos 2ks` vanishes identically — so *"the tracked loss equals `C_gamma E^4 I2 / 2 pi`"* passes
the moment the machinery exists, at any resolution anyone would choose, and says nothing about
whether the sampling is right. It is recorded as a **blindness** beside the gate it makes
vacuous, as J1 recorded that the structural gates cannot see a kick coefficient.

The two odd moments are the discriminating gates, asserted as **orders** rather than
tolerances (J2's lesson). `|cos|` has a jump in its first derivative, hence `n^-2`; `|cos|^3`
behaves as `|u|^3` near its zero and is `C^2`, so its jump is in the *third* derivative and it
converges two orders faster than the kink suggests. The three carry `h0^2`, `h0`, `h0^3`, so a
uniformly mis-scaled field cannot hide in all of them.

**The resolution that matters is per *period*, not per element.** `radiation_slices` is
therefore specified per period, default **32** (variance `1.6e-05`, count `1.6e-03`, mean
exact), with a floor of 3 — below which the mean itself stops being exact, and at `n = 2` the
samples land on the zeros of the cosine and the answer is 100% wrong. T2 paid for this lesson
without naming it: `slices = 64` on the ten-period probe magnet is 6.4 samples per period.

### The tracked loss is the integral's times the wiggle's own path lengthening

`U_tracked / U(I2) = 1 + theta^2/4` exactly, `1.280574e-05` on the probe magnet — and that
factor is the **same number** T1 ships as the element's `kick()` on `zeta`, divided by the
length. `I2 = int kappa^2 ds` integrates along the reference coordinate; a particle radiates
along the road it actually travels, and a wiggler is the only magnet in the package whose
on-design road is longer than its length (a bend's reference orbit *is* the arc, so its
`Delta zeta` is zero and it has no such factor). Two milestones' worth of unrelated machinery
landing on one number.

**The pre-committed gate 2 said the two agree at machine precision. They do not**, and the
difference is not an error in either. It also propagates: T2's staircase separation, carried
into the tracked variance, is `8 sqrt2/(3 pi) * (1 + theta^2/4) = 1.2004371` rather than the
bare `1.2004218`, because the staircase's own design orbit is exactly its length so the whole
of the path factor lands on the wiggler's side.

The uniform path-length share is *exact* at this order rather than approximate, and that is a
coincidence worth naming: the exact answer weights the local stretching by the local `kappa^2`,
`int cos^2 (1 + theta^2 sin^2/2) ds`, and `<cos^2 sin^2>/<cos^2> = 1/4` happens to equal
`<sin^2>/2`, which is the uniform factor.

### xtrack *can* arbitrate a wiggler's radiation — the first external check on axis T

T1 recorded that neither reference code has this magnet, and that is true of the **map**.
It is **not** true of the radiation: radiated energy needs only `|B_perp|` along the path, not
the plane the focusing lands in, and a stack of curved bends following the **segment mean** of
`h0 cos(ks)` *is* the wiggler's trajectory, discretised. So the arbiter that was structurally
blind to T1's physics is not blind to T3's.

- **On the shared stack the two codes agree at `5.33e-07`, constant in the pole count** across
  40 → 320 poles (measured `5.328, 5.325, 5.326, 5.324` in units of `1e-07`). That is the sum
  of the two owners already on record on xtrack's side — its pre-2019 elementary charge
  (`1.064e-08`) and its ultra-relativistic approximations (`2/gamma0^2 = 5.223e-07`). Nothing
  new appears when the field alternates twenty times, and nothing accumulates over 320
  elements. Both codes must be pinned to one kick per element (`integrator='uniform'`,
  `num_multipole_kicks=1`) first, exactly as B2's own cross-check requires.
- **The stack reaches the wiggler by a law.** The segment mean keeps `int h ds` (the geometry,
  and hence the closing of the trajectory) exact at every resolution but under-counts
  `int h^2 ds` by the field's own variance across a segment, so the loss approaches
  `C_gamma E^4 I2 / 2 pi` **from below** as `(k ds)^2/12`. Measured deficit/law:
  `0.9797, 0.9950, 0.9992` at 8, 16, 32 poles per period.
- **And the arbiter's reach is stated rather than implied.** A Richardson step on two
  resolutions lands on the shipped wiggler's tracked loss to `3.1e-05`, a hundredfold
  improvement on the raw `3.2e-03`; a *second* stage only reaches `1.4e-05`, so the remainder
  is not a clean `ds^4` — the segment mean is taken across a field that changes sign twenty
  times, and at those zeros there is no smooth even expansion. The consequence: **xtrack is
  blind to the `theta^2/4` path lengthening**, which stays an accsim-internal statement gated
  in the analytic suite. A clean reference half and a half the arbiter cannot see — S2's and
  T1's shape on this axis.

### What lifted, and the one refusal left standing

- **Radiation tracking** through a `Wiggler` works in all three physical models
  (`"mean"`, `"quantum"`, `"photons"`). The graininess is a sum over the body: independent
  per-sample variances add, and the photon counts are per-sample Poissons.
- **`taper()`** on a ring containing a wiggler works. Its last cause was this accessor,
  verified rather than assumed — `taper()` reaches `closed_orbit_6d(radiation="mean")`, which
  tracks — and T2 had already put `Wiggler` in `tapering._STRENGTHS`. What such a ring meets
  now is the ordinary requirement of every radiating ring, an RF cavity to close the 6D orbit
  against.
- **`normalized_field` still raises**, and the message now names the reason: what arrives
  there is `accsim.spin`, which samples the field once at the mid-point exactly as radiation
  used to, and would precess a spin through a field that averages to zero. **Spin through a
  wiggler is T4** — radiation is an integral along the path, spin is a composition of
  rotations in which order matters and the net over a period is not the average field. Same
  accessor, different physics, different gates. The test asserting the refusal is written to
  fail the day T4 lands.

### Out of scope for T3

Spin (T4). Sub-period tracking of the *trajectory* — the map stays the period average, for the
reason at the top of this section. Undulator radiation spectra and coherence, which are
...
- **And a `nicepytest.py` run can be starved outright, by design (2026-09-06, S1).** With
  three other sessions' suites saturating the box (`_Total` processor time pinned at 100%), a
  `BelowNormal` run sat at **2.6 s of CPU after 20 minutes** — twice, on two different
  selections — and never reached its first test. That is the wrapper doing exactly what it
  exists to do, not a hang: the check is the process's *CPU counter*, not the clock. It
  eventually runs. To keep iterating in the meantime, drive the test functions directly
  (import the module, call them, resolve the `ref` fixture and any `parametrize` marks by
  hand) — the analytic files' only shared fixture is `tests/conftest.py`'s feature-switch
  reset, so a direct call is faithful. **The real run still goes through `nicepytest.py`.**
